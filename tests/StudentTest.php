...
public function testGetOrCreateForum()
{
    $submissionID = 123;
    $instructorID = 456;
    $studentID = 789;

    $forumManager = new Forums($this->db);

    $forumID1 = $forumManager->getOrCreateForum($submissionID, $instructorID, $studentID);

    $this->assertTrue($forumID1 > 0, "A new forum should be created and return a valid ID.");

    $forumID2 = $forumManager->getOrCreateForum($submissionID, $instructorID, $studentID);

    $this->assertEquals($forumID1, $forumID2, "The existing forum ID should be returned.");
}
