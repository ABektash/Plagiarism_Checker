    ...
    if (lastForumLoad != forumID) {
        ...
        try {
            const response = await fetch(
                `/Plagiarism_Checker/public/Forums/submit?submitGetForum=true&forumID=${forumID}`,
                {
                    method: "GET",
                }
            );
            ...
            let chatname;
            if (UserType_id === 1) {
                chatname = `Dr. ${data.instructorName} & ${data.studentName}`;
            ...
            messages.forEach((msg) => {
                const senderClass = (parseInt(msg.SenderID, 10) === UserID || (UserType_id === 1 && parseInt(msg.sender_UserType_id, 10) === 2)) ? "right" : "left";

                const sentTime =
                    msg.Sentat !== "Unknown time" && msg.Sentat
                        ? (() => {
                            const fullTime = new Date(msg.Sentat);
                            let hours = fullTime.getHours();
                            const minutes = fullTime
                                .getMinutes()
                                .toString()
                                .padStart(2, "0");
                ...
                const messageHTML = `
                    ${lastMessageDate !== relativeDate
                        ? `<div class="date-separator"><span>${relativeDate}</span></div>`
                        : ""
                    }
                    <div class="chat-message ${senderClass}">
                        <div class="message-content">
                            <div class="message-text">${msg.Messagetext
                    }</div>        
...
const sendMessage = async (event) => {
    ...
    if (message) {
        ...
        formData.append(
            "senderID",
            messageForm.querySelector('[name="senderID"]').value
        );
